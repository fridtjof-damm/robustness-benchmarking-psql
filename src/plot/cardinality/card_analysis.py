...
def prepare_data(file_path):
    ...
    df.columns = ['query_id', 'node_type', 'filters', 'exec_time', 'card_ratio']
...
def plot_cumulative_cardinality_errors(df):
    ...
    df = df.assign(node_type=df['node_type'].str.split(', ')).explode('node_type')
    
    # Sort by execution time
    df = df.sort_values(by='exec_time')
    
    # Calculate cumulative sum of cardinality errors
    df['cumulative_card_error'] = df.groupby('node_type')['card_error'].cumsum()
    
    plt.figure(figsize=(15, 8))
    
    # Create line plot
    sns.lineplot(x='exec_time', y='cumulative_card_error', hue='node_type', data=df, marker='o')
    
    # Customize plot
    plt.xlabel('Execution Time')
    plt.ylabel('Cumulative Cardinality Error (log2)')
    plt.title('Cumulative Cardinality Errors by Node Type')
    plt.legend(title='Node Type', bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.show()

# Usage
df = prepare_data('/Users/fridtjofdamm/Documents/thesis-robustness-benchmarking/results/job/all_job.csv')
