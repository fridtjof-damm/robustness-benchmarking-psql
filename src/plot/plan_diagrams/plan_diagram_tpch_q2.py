...
import os
import query_plan_similarity as qps

# not correct maybe
directory = '/Users/fridtjofdamm/Documents/thesis-robustness-benchmarking/results/job/15a'
query_info = qps.info_qplan(directory)
plan_categories = qps.categorize_plans(query_info)
...
for query_id, info in query_info.items():
    for node_type, filters, execution_time, cardinality in zip(info[0], info[1], info[2], info[3]):
        ...
        # Use the actual rows from cardinality
        data.append((a_value, b_value, cardinality[1]))
...
for a_value, b_value, cardinality in data:
    ...
    heatmap_data[b_index, a_index] = min(
        cardinality, percentile_95_cardinality)  # Clip cardinality values
...
norm = Normalize(vmin=np.min(
    heatmap_data[heatmap_data > 0]), vmax=percentile_95_cardinality)
...
# Set rotation to 0 for horizontal alignment
ax.set_xticklabels(a_values, rotation=0)
...
# Ensure labels are divisible by 100
y_labels = [label if label % 100 == 0 else '' for label in y_labels]
...
output_path = os.path.join(
    output_dir, 'skew_res_cardinalities_heatmap_95_clip.pdf')
