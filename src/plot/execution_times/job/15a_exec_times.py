...
for a_value, b_value, execution_time in data:
    ...
    heatmap_data[b_index, a_index] = min(
        execution_time, percentile_90_execution_time)  # Clip execution time values
...
norm = mpl.colors.Normalize(vmin=np.min(
    heatmap_data[heatmap_data > 0]), vmax=percentile_90_execution_time)
...
output_path = os.path.join(
    output_dir, 'skew2_execution_time_heatmap_90_clip.pdf')
